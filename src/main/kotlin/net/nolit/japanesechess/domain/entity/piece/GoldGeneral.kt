...
class GoldGeneral(override val position: Position, override val isBlack: Boolean): Piece(position, isBlack) {
    ...
    override fun setPosition(position: Position): Piece {
        return GoldGeneral(position, isBlack)
    }
    ...
    override fun promote(): Piece {
        throw IllegalCallerException("GoldGeneral cannot promote")
    }

    override fun toPieceInHand(): PieceInHand {
        return PieceInHand (getName()) { position -> GoldGeneral(position, !isBlack) }
    }
